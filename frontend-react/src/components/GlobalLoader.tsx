...
export default function GlobalLoader() {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 text-white overflow-hidden">
      ...
      <div className="relative flex flex-col items-center justify-center p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl shadow-2xl animate-in fade-in zoom-in duration-500">
        
        {/* Animated Icons Container */}
        <div className="relative flex items-center justify-center w-24 h-24 mb-6">
          <div className="absolute inset-0 bg-emerald-500/20 rounded-full animate-ping opacity-75"></div>
          <div className="absolute bg-slate-800 rounded-full w-20 h-20 flex items-center justify-center border border-slate-700 shadow-inner z-10">
            {/* Gear spinning continuously */}
            <Settings className="w-10 h-10 text-emerald-400 animate-spin z-20" style={{ animationDuration: '3s' }} />
          </div>
        ...
        <div className="text-center space-y-2">
          ...
          <div className="flex items-center justify-center space-x-2 text-slate-400 font-mono text-sm">
        ...
        <div className="w-full h-1.5 bg-slate-800 rounded-full mt-8 overflow-hidden relative">
          <div className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-full w-1/2 animate-[progress_1.5s_ease-in-out_infinite_alternate]"></div>
          <style>{`
            @keyframes progress {
