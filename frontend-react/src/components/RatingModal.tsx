    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-2xl bg-[#1a2333] border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]"
        >
          {/* Header */}
          <div className="p-6 border-b border-slate-700/50 flex justify-between items-start shrink-0">
            <div>
              <h2 className="text-2xl font-bold text-white mb-1">Student Reviews</h2>
              <p className="text-sm text-emerald-400 font-medium max-w-sm truncate" title={`for ${courseName}`}>for {courseName}</p>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto">
            <div className="flex flex-col md:flex-row gap-8 mb-8 pb-8 border-b border-slate-700/50">
              <div className="flex flex-col items-center justify-center min-w-[120px]">
                <div className="text-5xl font-extrabold text-white mb-2">{ratingValue.split(' ')[0] || "4.8"}</div>
                ...
                <div className="text-sm text-slate-400 whitespace-nowrap">{ratingValue.replace('4.8/5 ', '') || "ratings"}</div>
              </div>
              
              <div className="flex-1 space-y-2 w-full">
                {[5, 4, 3, 2, 1].map(star => (
                  <div key={star} className="flex items-center text-sm">
                    <span className="w-14 text-slate-400 whitespace-nowrap">{star} stars</span>
            ...
            {isEnrolled && !showReviewForm && (
              <div className="mb-6 flex justify-end">
                <button 
                  ...
                  className="flex items-center space-x-2 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-500 border border-emerald-500/20 px-4 py-2 rounded-lg font-medium transition-colors"
                >
            ...
            {showReviewForm && (
              <motion.form 
                ...
                className="mb-8 bg-[#111827] p-5 rounded-xl border border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)]"
              >
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="font-semibold text-white">Your Rating</h3>
                  <div className="flex space-x-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star className={`w-6 h-6 ${star <= newReviewStars ? 'text-yellow-400 fill-current' : 'text-slate-600'}`} />
                      </button>
                    ))}
                  </div>
                </div>
                <div className="mb-4">
                  <textarea
                    ...
                    required
                    className="w-full bg-[#1a2333] border border-slate-700 rounded-lg p-3 text-white focus:outline-none focus:border-emerald-500 transition-colors resize-none h-24"
                  />
                </div>
                <div className="flex justify-end space-x-3">
                  ...
                  <button 
                    type="submit"
                    className="flex items-center space-x-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-lg font-medium transition-colors shadow-lg"
                  >
            ...
            <div className="space-y-4">
              {reviews.map(review => (
                <div key={review.id} className="bg-[#111827] p-5 rounded-xl border border-slate-800 transition-colors hover:border-slate-700">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border border-emerald-500/10 text-emerald-500 rounded-full flex items-center justify-center font-bold">
                    ...
                    <div className="flex text-yellow-400 space-x-0.5 mt-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < review.stars ? 'fill-current' : 'text-slate-700 fill-slate-700'}`} />
                      ))}
                    </div>
                  </div>
