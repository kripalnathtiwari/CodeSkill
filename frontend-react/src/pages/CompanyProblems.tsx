          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6 mt-10">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Practice Problems</h3>
                ...
                <div className="relative w-full sm:w-64">
                  ...
                  <input
                    ...
                    className="w-full bg-surface dark:bg-background border border-border dark:border-border rounded-lg pl-9 pr-4 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-blue-500/20 text-text-primary dark:text-text-inverse placeholder-slate-400 transition-all"
                  />
                </div>
              </div>

              {/* Section Tabs (MCQ / Coding) */}
              <div className="flex border-b border-border dark:border-border/60">
              ...
              <div className="space-y-4 pt-2">
                {isLoading ? (
                  <div className="text-center py-12 text-text-muted">Loading problems...</div>
                ) : activeQuestions.length === 0 ? (
                  <div className="text-center py-12 text-text-muted glass-card rounded-2xl border border-border dark:border-border/40">
                    No {selectedSection === "MCQ" ? "MCQ questions" : "coding problems"} found.
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-4">
                    {activeQuestions.map((q: any, idx: number) => {
                      const isMcq = q.type === 'MCQ' || q.questionType === 'MCQ';
                      return (
                        <div key={q._id || idx} className="bg-surface dark:bg-background rounded-xl p-5 border border-border dark:border-border/80 shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div>
                            <h4 className="font-bold text-text-primary text-base line-clamp-2">
                              {q.title || q.questionText || "Untitled Question"}
                            </h4>
                            <div className="flex flex-wrap items-center gap-3 mt-3 text-xs">
                              <span className={`px-2.5 py-1 rounded-md font-bold tracking-wide ${
                                q.difficulty === 'EASY' || q.difficulty === 'Easy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 
                                q.difficulty === 'HARD' || q.difficulty === 'Hard' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 
                                'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
                              }`}>
                                {(q.difficulty || "MEDIUM").toUpperCase()}
                              </span>
                              <span className="flex items-center text-text-muted font-medium">
                                {isMcq ? (
                                  <><CheckCircle2 className="w-3.5 h-3.5 mr-1" /> MCQ</>
                                ) : (
                                  <><Code2 className="w-3.5 h-3.5 mr-1" /> Coding</>
                                )}
                              </span>
                              {(q.companyTags || q.companies)?.length > 0 && (
                                <span className="flex items-center text-text-muted font-medium border-l border-border pl-3">
                                  <Building2 className="w-3.5 h-3.5 mr-1" />
                                  {Array.isArray(q.companyTags || q.companies) 
                                    ? ((q.companyTags || q.companies)[0].name || (q.companyTags || q.companies)[0])
                                    : (q.companyTags || q.companies)}
                                </span>
                              )}
                            </div>
                          </div>
                          <Link
                            to={isMcq ? `/practice/mcq/${q._id}` : `/practice/coding/${q._id}`}
                            className="shrink-0 bg-blue-50 dark:bg-blue-900/20 hover:bg-[#0066cc] hover:text-white text-[#0066cc] dark:text-blue-400 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all text-center"
                          >
                            Solve Problem
                          </Link>
                        </div>
                ...
                {hasMore && (
                  <div className="text-center pt-6">
                    <button 
                      ...
                      className="inline-flex items-center justify-center space-x-2 bg-surface dark:bg-background border border-border dark:border-border/80 hover:bg-slate-50 dark:hover:bg-slate-800 text-text-primary px-6 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-sm"
                    >
                      {loadingMore ? (
                        <>
