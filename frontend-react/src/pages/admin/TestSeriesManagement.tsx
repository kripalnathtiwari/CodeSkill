...
export default function TestSeriesManagement() {
  ...
  const [customTests, setCustomCompanies] = useState<TestCardData[]>([]);

  // Selected Company for managing its questions (null = directory view)
  const [selectedTest, setSelectedCompany] = useState<string | null>(null);

  // Search filter states
  const [searchTestQuery, setSearchCompanyQuery] = useState("");
  const [searchQuestionQuery, setSearchQuestionQuery] = useState("");

  // Modals state
  const [showAddTestModal, setShowAddCompanyModal] = useState(false);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const savedCompanies = localStorage.getItem("admin_test_series_names");
        if (savedCompanies) {
        ...
        // Save the merged companies to state if we had a state for it, but we can just use useMemo below based on customTests.
        // Actually, we'll store allCompanies directly in state.
        setAllCompaniesState(mergedTests);
      } catch (err) {
  ...
  const [allTestsState, setAllCompaniesState] = useState<TestCardData[]>(DEFAULT_TESTS);

  // Filtered companies based on search
  const filteredTests = useMemo(() => {
    if (!searchTestQuery) return allTestsState;
    return allTestsState.filter(c =>
      c.name.toLowerCase().includes(searchTestQuery.toLowerCase()) ||
      c.category.toLowerCase().includes(searchTestQuery.toLowerCase())
    );
  }, [allTestsState, searchTestQuery]);
  ...
  }, [selectedTest, searchQuestionQuery]);
  ...
  const fetchTestQuestions = async () => {
    ...
    try {
      ...
      const savedQs = localStorage.getItem("admin_test_series_problems");
      if (savedQs) {
  ...
  const testQuestions = problems;

  // We skip detailed counts in directory view for performance.
  const getTestStats = (companyName: string) => {
  ...
  const handleAddTest = () => {
    ...
    const exists = allTestsState.some(c => c.name.toLowerCase() === newTestName.trim().toLowerCase());
    if (exists) {
    ...
    const updated = [...customTests, newComp];
    setCustomCompanies(updated);
    localStorage.setItem("admin_test_series_names", JSON.stringify(updated));
    setNewCompanyName("");
  ...
  const handleDeleteTest = (testNameVar: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm(`Delete "${testNameVar}" from companies list?`)) return;
    const updated = customTests.filter(c => c.name.toLowerCase() !== testNameVar.toLowerCase());
    setCustomCompanies(updated);
    localStorage.setItem("admin_test_series_names", JSON.stringify(updated));
    if (selectedTest === testNameVar) {
  ...
  const handleSaveMCQ = async () => {
    ...
    const questionObj = {
      _id: editingProblem?._id || "custom-testseries-mcq-" + Date.now() + Math.random().toString(36).substr(2, 4),
      id: editingProblem?.id || "custom-testseries-mcq-" + Date.now() + Math.random().toString(36).substr(2, 4),
      title: mcqTitle.trim(),
      ...
      testSeriesTags: [{ name: selectedTest }],
      testSeries: [{ name: selectedTest }],
      options: {
        A: optionA.trim(),
        B: optionB.trim(),
        C: optionC.trim(),
    ...
    const savedRaw = localStorage.getItem("admin_test_series_problems");
    ...
    localStorage.setItem("admin_test_series_problems", JSON.stringify(newSavedList));
  ...
  const handleSaveCoding = async () => {
    ...
    const questionObj = {
      _id: editingProblem?._id || "custom-testseries-coding-" + Date.now() + Math.random().toString(36).substr(2, 4),
      id: editingProblem?.id || "custom-testseries-coding-" + Date.now() + Math.random().toString(36).substr(2, 4),
  ...
  const handleDeleteQuestion = async () => {
    ...
    if (savedRaw) {
      ...
      localStorage.setItem("admin_test_series_problems", JSON.stringify(filtered));
    }
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredTests.map((comp) => {
                const stats = getTestStats(comp.name);
                const isCustom = customTests.some(c => c.name.toLowerCase() === comp.name.toLowerCase());
                return (
                  <div
                    key={comp.name}
                    onClick={() => {
                  ...
                  >
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <TestLogoHeader name={comp.name} iconType={comp.iconType} />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-3">
                <h1 className="text-2xl font-bold text-text-primary flex items-center">
                  <TestLogoHeader
                    ...
                    logoUrl={allTestsState.find(c => c.name.toLowerCase() === selectedTest.toLowerCase())?.logoUrl}
                  />
              ...
              <p className="text-sm text-text-muted">
                Add and manage MCQ questions or Coding problems specifically for {selectedTest}.
              </p>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/60 text-sm">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-text-muted">
                        No questions found for {selectedTest}.
                      </td>
                    </tr>
                  ) : (
                    testQuestions.map((q) => {
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-border rounded-2xl max-w-md w-full p-6 space-y-6 shadow-2xl">
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl text-sm font-semibold text-text-muted hover:text-text-inverse bg-slate-100 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
              >
                Cancel
              </button>
              <button
