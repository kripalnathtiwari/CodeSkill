    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          <button 
            ...
            className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-text-secondary px-4 py-2 rounded-xl font-bold transition-all"
          >
            <span>Refresh</span>
          </button>
        </div>
      ...
      <div className="bg-surface dark:bg-[#111827] border border-border rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-border flex justify-between items-center bg-slate-900/50">
          ...
          <span className="text-sm font-medium text-text-muted">Showing {filtered.length} users</span>
        </div>
            ...
            <table className="w-full text-left text-sm text-text-secondary">
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.map(activity => (
                  <tr key={activity.id} className="hover:bg-slate-200/30 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col space-y-1">
                        ...
                        <div className="flex items-center space-x-3 text-xs mt-1">
                          <span className="text-primary bg-primary/10 px-2 py-0.5 rounded">Easy: {activity.dsa.easy}</span>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-text-muted">No user activity found.</td>
                  </tr>
                )}
              </tbody>
            </table>
